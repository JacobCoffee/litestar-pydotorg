...
class TestEmailTemplateControllerRoutes:
    ...
    async def test_list_templates_by_type(
        ...
        response = await client.get(
            "/api/admin/email-templates/?template_type=transactional"
        )
        assert response.status_code in (200, 401, 403, 500)
    ...
    async def test_get_template_by_name(
        ...
        template = await _create_email_template_via_db(
            postgres_uri, internal_name="unique-template-name"
        )
        response = await client.get(
            f"/api/admin/email-templates/by-name/{template.internal_name}"
        )
        assert response.status_code in (200, 401, 403, 500)

    async def test_get_template_by_name_not_found(self, client: AsyncTestClient) -> None:
        response = await client.get(
            "/api/admin/email-templates/by-name/non-existent-template"
        )
        assert response.status_code in (401, 403, 404, 500)
    ...
    async def test_update_template(
        ...
        response = await client.patch(
            f"/api/admin/email-templates/{template.id}", json=data
        )
        assert response.status_code in (200, 401, 403, 500)
    ...
    async def test_validate_template(
        ...
        response = await client.post(
            f"/api/admin/email-templates/{template.id}/validate"
        )
        assert response.status_code in (200, 401, 403, 500)
    ...
    async def test_preview_template(
        ...
        data = {"name": "Test User"}
        response = await client.post(
            f"/api/admin/email-templates/{template.id}/preview", json=data
        )
        assert response.status_code in (200, 401, 403, 500)
...
class TestEmailLogControllerRoutes:
    ...
    async def test_list_logs_with_limit(
        self, client: AsyncTestClient, postgres_uri: str
    ) -> None:
        for _ in range(3):
            await _create_email_log_via_db(postgres_uri)
    ...
    async def test_list_logs_by_recipient(
        self, client: AsyncTestClient, postgres_uri: str
    ) -> None:
        await _create_email_log_via_db(postgres_uri)
        response = await client.get(
            "/api/admin/email-logs/?recipient=test@example.com"
        )
        assert response.status_code in (200, 401, 403, 500)

    async def test_list_logs_by_template(
        self, client: AsyncTestClient, postgres_uri: str
    ) -> None:
        await _create_email_log_via_db(postgres_uri, template_name="test-template")
        response = await client.get(
            "/api/admin/email-logs/?template_name=test-template"
        )
        assert response.status_code in (200, 401, 403, 500)

    async def test_list_failed_logs(
        self, client: AsyncTestClient, postgres_uri: str
    ) -> None:
...
class TestMailingValidation:
    ...
    async def test_create_template_missing_internal_name(
    ...
    async def test_create_template_missing_subject(
        self, client: AsyncTestClient
    ) -> None:
        data = {
    ...
    async def test_create_template_missing_content(
        self, client: AsyncTestClient
    ) -> None:
        data = {
            "internal_name": "test-template",
