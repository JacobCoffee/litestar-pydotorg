...
| ~~**CODEBASE-WIDE: Audit htmx 405 errors**~~ | All templates | ~~HIGH~~ FIXED | ~~Search all templates for `hx-post`, `hx-put`, `hx-delete`, `hx-patch` and verify corresponding route handlers exist.~~ **Fixed (2025-12-01)**: Comprehensive audit completed. Added missing endpoints: `PUT /api/auth/profile`, `POST /api/auth/change-password`, `PUT /api/auth/settings`, `DELETE /api/auth/account`, `POST /api/v1/sponsors/apply`, `POST /api/nominations/{slug}/nominate`. Fixed template paths (`resend-verification` instead of `verify-email`). |
...
| ~~**`/blogs` feed prioritization**~~ | `/blogs` | ~~MEDIUM~~ FIXED | ~~Should prioritize official Python blogs (Python Insider, PSF Blog) over aggregated feeds.~~ **Fixed (2025-12-01)**: Added `priority` field to Feed model with migration. Feeds sorted by priority DESC. Admin UI allows editing priority per feed. |
...
| ~~**`/events/submit` no confirmation**~~ | `/events/submit` | ~~MEDIUM~~ FIXED | ~~No toast or confirmation after submitting event.~~ **Fixed (2025-12-01)**: Added success toast notification with DaisyUI alert styling on successful form submission. |
